...
const Landing = () => {
  const [showWelcome, setShowWelcome] = useState(() => {
    const seen = sessionStorage.getItem('bhu-welcome-seen');
    return !seen;
  });

  const handleWelcomeComplete = useCallback(() => {
    setShowWelcome(false);
    sessionStorage.setItem('bhu-welcome-seen', '1');
  }, []);
    ...
    <>
      ...
      <div className="min-h-screen relative z-10">
        ...
        <section className="relative min-h-screen flex items-center overflow-hidden">
          ...
          <div className="relative z-10 container mx-auto px-4 py-20">
            <div className="max-w-2xl">
              <motion.div
                ...
                transition={{ duration: 0.6, delay: showWelcome ? 3.5 : 0 }}
              ...
              <motion.h1
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: showWelcome ? 3.6 : 0.1 }}
              ...
              <motion.p
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: showWelcome ? 3.7 : 0.2 }}
              ...
              <motion.div
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: showWelcome ? 3.8 : 0.3 }}
              ...
              <motion.div
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: showWelcome ? 3.9 : 0.4 }}
          ...
          <motion.div className="absolute right-10 top-1/4 text-5xl floating hidden lg:block" initial={{ opacity: 0 }} animate={{ opacity: 0.8 }} transition={{ delay: 0.8 }}>🍛</motion.div>
          <motion.div className="absolute right-32 bottom-1/3 text-4xl floating-delayed hidden lg:block" initial={{ opacity: 0 }} animate={{ opacity: 0.6 }} transition={{ delay: 1.2 }}>🍗</motion.div>
        </section>
        ...
        {/* Footer */}
        <footer className="bg-foreground/[0.03] border-t border-border/50 py-8 relative z-10">
          <div className="container mx-auto px-4 text-center">
            ...
            <p className="text-sm text-muted-foreground">Created by emmy-brain-codes 🛰️</p>
            <p className="text-xs text-muted-foreground/60 mt-1">© 2026 – For Bingham University Students Only</p>
          </div>
        </footer>
      </div>
    </>
